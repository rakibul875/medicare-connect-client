...
const DoctorDetailsCard = ({doctor}) => {
  ...
  return (
    <div className="max-w-[360px] w-full bg-white rounded-[2.5rem] border border-gray-100/70 p-8 shadow-sm flex flex-col items-center text-center my-6">
      ...
      <div className="w-full text-left space-y-2 pt-2 border-t border-gray-50">
        ...
        <p className="text-sm font-medium text-gray-500 leading-relaxed tracking-normal text-justify sm:text-left">
          {} specializes in non-invasive cardiology and preventative
          heart care. With over a decade of clinical excellence at top-tier
          institutions, she combines advanced diagnostic technology with a
          patient-centric approach to cardiovascular health.
        </p>
        <p className="text-2xl text-slate-500">Consultation Fee</p>
        <p className="text-cyan-700 font-bold text-3xl flex items-center gap-1"><HiCurrencyDollar />{doctor.consultationFee}</p>
      </div>   
    </div>
  );
};
